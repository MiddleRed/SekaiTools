...
namespace SekaiTools.UI.Radio
{
    public class Radio_MusicLayer : MonoBehaviour
    {
        ...
        {
            textMusicTitle.text = $"���ڲ��� - {playMusic.musicData.masterMusic.composer}\n{playMusic.musicData.masterMusic.title}";
        ...
        public void RefreshPlaylist()
        {
            ...
            if (musicQueue.Count == 0)
            {
                strQueueLength = "��ǰ�����б�Ϊ��";
                strNextMusic = "��һ�� - �������";
            }
            else if (musicQueue.Count == maxPlaylistLength)
            {
                strQueueLength = $"��ǰ�����б�����Ϊ {musicQueue.Count}�������б�����";
                strNextMusic = $"��һ�� - {musicQueue.Peek().musicData.masterMusic.title}";
            }
            else
            {
                strQueueLength = $"��ǰ�����б�����Ϊ {musicQueue.Count}";
                strNextMusic = $"��һ�� - {musicQueue.Peek().musicData.masterMusic.title}";
            }
        ...
        public RadioMessage OrderMusic(MusicOrderInfo musicOrderInfo,string userName)
        {
            ...
            if (musicInQueue == null||musicInQueue.vocalData==null)
                return new RadioMessage(userName, MessageType.error, "δ�ҵ����������ĸ���");
            if (bannedMusics.Contains(musicInQueue.musicData.id))
                return new RadioMessage(userName, MessageType.error, $"�˵�̨���ɲ��� {musicInQueue.musicData.masterMusic.title}");
            MusicInQueue[] musicInArray = musicQueue.ToArray();
            foreach (var music in musicInArray)
            {
                if(music.musicData == musicInQueue.musicData)
                    return new RadioMessage(userName, MessageType.error, $"�����б������� {musicInQueue.musicData.masterMusic.title}");
            }
            ...
            RefreshPlaylist();
            Debug.Log($"@{userName} ���ɹ� {musicInQueue.musicData.masterMusic.title}��Vo. {string.Join("��", musicInQueue.vocalData.singers)}");
            return new RadioMessage(userName, MessageType.success, $"���ɹ� {musicInQueue.musicData.masterMusic.title}");
        }
